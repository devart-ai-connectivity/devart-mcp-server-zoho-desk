// --------------------------------------------------------------------------
// <copyright file="PrimaryKeysTool.cs" company="Devart">
//
...
{
  public abstract class PrimaryKeysTool(McpConfiguration serverConfiguration) : McpTool(serverConfiguration)
  {
    protected override string Name => "get_primary_keys";

    protected override string Description => string.Format(McpResources.PrimaryKeysTool_Description, ServerConfiguration.SourceDisplayName);
      ...
      CancellationToken cancellationToken)
    {
      ...
      var markdownTable = table?.ToMarkdown(metadata.PrimaryKeysColumnsMapping, null);
      var quotedFullName = formatter.FormatName(schema, tableName, configuration, connection);
      var header = string.Format(McpResources.PrimaryKeysTool_OutputHeader, quotedFullName);
